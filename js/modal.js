 ...
 *   showToast(message, type?, ms?)   → void
 */

// ─── CSS (injected once on module load) ──────────────────────────────────────
...
(function injectModalStyles() {
    ...
    style.textContent = `
        ...
        #lsc-modal-message {
            ...
            margin: 0 0 20px 0; line-height: 1.5; white-space: pre-wrap;
        }
        #lsc-modal-buttons { display: flex; justify-content: flex-end; gap: 10px; flex-wrap: wrap; }
        .lsc-modal-btn { padding: 9px 18px; border: none; border-radius: 4px; font-size: 14px; cursor: pointer; font-family: Arial, sans-serif; }
        ...
        #lsc-toast-container {
            ...
            z-index: 50000; display: flex; flex-direction: column;
...
export function showConfirm(message, options = {}) {
    return new Promise(resolve => {
        ...
        const close = result => { backdrop.classList.add('hidden'); resolve(result); };
...
export function showAlert(message, options = {}) {
    return new Promise(resolve => {
        ...
        const close = () => { backdrop.classList.add('hidden'); resolve(); };
